...
class WorkerService:
    def __init__(self, settings: Settings) -> None:
        ...
        self.logger = configure_logging("multimedia_ana.worker", settings.worker_log_level, settings.worker_log_file)
    ...
    def run(self) -> None:
        ...
        self.logger.info("Worker starting with idle_timeout=%ss", self.settings.worker_idle_timeout_seconds)
        ...
        while True:
            ...
            if idle_for >= self.settings.worker_idle_timeout_seconds:
                self.logger.info("Worker idle for %.1fs, exiting", idle_for)
