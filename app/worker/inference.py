...
class VideoVLRunner:
    ...
    def _prepare_inputs(self, request: dict) -> tuple[dict, dict]:
        ...
        video_path = Path(request["video_uri"]).resolve()
        max_frames = min(
            request.get("max_frames", self.settings.video_vl_transformers_max_frames),
                ...
                "content": [
                    {
                        ...
                        "fps": request.get("sample_fps", 1.0),
                    ...
                    {
                        "type": "text",
                        "text": self.settings.video_vl_prompt_template,
                    },
                ],
            }
        ]
    ...
    def analyze(self, request: dict, output_path: Path) -> Path:
        ...
        with torch.inference_mode():
            generated_ids = self._model.generate(
                **inputs,
                do_sample=True,
                temperature=self.settings.video_vl_temperature,
                top_p=self.settings.video_vl_top_p,
                top_k=20,
                repetition_penalty=1.0,
                max_new_tokens=self.settings.video_vl_max_tokens,
            )
        ...
        result = {
            ...
            "prompt_template": self.settings.video_vl_prompt_template,
            "profile": request.get("profile"),
            "language": request.get("language"),
            "max_frames": metadata["max_frames"],
            "sample_fps": request.get("sample_fps"),
        ...
        self.logger.info("Wrote analysis result to %s", output_path)
        return output_path
